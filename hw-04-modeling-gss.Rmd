...
set.seed(242)
advfront <- sample(c("Strongly agree", "Agree", "Don't know", "Disagree", "Strongly disagree", NA), 100, replace = TRUE)


advfront <- as_tibble(advfront)


advfront <- advfront %>%
  mutate(agree = ifelse(str_detect(value, "[Ss]trongly agree|[Aa]gree"), "Agree", NA),
         not_agree = ifelse(!is.na(value) & is.na(agree), "Not agree", NA)) %>%
  
  select(agree, not_agree) %>%
  
  mutate(agree = factor(agree, levels = c("Agree", "Not agree")),
         not_agree = factor(not_agree, levels = c("Agree", "Not agree"))) %>%
  
  summarise(agree_count = sum(!is.na(agree)),
            not_agree_count = sum(!is.na(not_agree)))
...
set.seed(331)
polviews <- sample(c("Extremely liberal", "Liberal", "Slightly liberal", "Moderate", "Slightly conservative", "Conservative", "Extremely conservative", NA), 100, replace = TRUE)

polviews <- as_tibble(polviews)


polviews <- polviews %>%
  mutate(new_level = case_when(
    str_detect(value, "[Ll]iberal") ~ "Liberal",
    str_detect(value, "[Cc]onservative") ~ "Conservative",
    TRUE ~ "Moderate"
  )) %>%
  ...
  mutate(new_level = factor(new_level, levels = c("Conservative", "Moderate", "Liberal"))) %>%
...
folds <- vfold_cv(gss16_train, v = 5)
...
***I was unable to determine which models works better due to not being able to summarize either of their metrics.***
